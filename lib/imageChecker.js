...
// Image checker with optional Gemini Vision (uses GEMINI_API_KEY) and
...
export async function runImageChecks({ imageFile, imageUrl, platform }) {
  ...
  // --- Gemini Vision (if key present and image provided) ---
  let vision = null;
  if (hasGeminiKey && (imageFile || imageUrl)) {
    try {
      ...
      if (imagePart) {
        const prompt = [
          ...
          "- beforeAfterDetected: boolean (true if image shows before/after comparison)",
          ...
          "- ocrText: short text (<= 160 chars) you can read from the image",
          "- annotatedRegions: array of { x: percent string (e.g. '10%'), y: percent string, width: percent string, height: percent string, label: string } for notable regions that violate policies (before/after panels, nudity areas, negative imagery regions). Include ALL regions that might violate ad policies.",
      ...
      console.error(
        "Gemini vision check failed, falling back to heuristics",
        err
      );
    }
  }
...
async function toGeminiImagePart(imageFile, imageUrl) {
  // If we have a file upload, convert to base64
  ...
  // (Gemini doesn't support direct URL references for images)
